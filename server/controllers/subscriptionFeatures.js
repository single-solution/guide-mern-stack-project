...
const getSubscriptionFeatures = async (request, response) => {
	try {
		const { _id: itemID, page, limit, count } = request.query;

		if (count === "true") {
			const totalRecords = await subscriptionFeatures.countDocuments({});
		...
		const dbPayload = await subscriptionFeatures
...
const createSubscriptionFeature = async (request, response) => {
	try {
		...
		const payloadToSaveInDB = new subscriptionFeatures({
			...payload,
...
const updateSubscriptionFeature = async (request, response) => {
	try {
		const payload = request.body;
		const { userID: authenticatingUserID } = request.jwtPayload;
		...
		const updatedDBPayload = await subscriptionFeatures.findOneAndUpdate(
			{ _id: payload._id },
			{ $set: { ...payload, subscriptions: payload?.subscriptions || [], updatedBy: authenticatingUserID } },
			{ new: true },
		);
...
const deleteSubscriptionFeature = async (request, response) => {
	try {
		...
		const deletedDBPayload = await subscriptionFeatures.findOneAndDelete({ _id: deleteItemID }, { new: true });
